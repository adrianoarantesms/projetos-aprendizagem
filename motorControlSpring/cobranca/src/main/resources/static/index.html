...
<body ng-controller="RestauranteController">
	<div class="well"
		style="width: 700px; margin-top: 60px; margin-left: auto; margin-right: auto;">
		<input type="text" class="form-control" ng-model="criterio"
			placeholder="O que você está procurando?" /><br />
		<table class="table table-striped"
			ng-show="(cardapio | filter:criterio).length != 0">
			<tr>
				<th>Descrição</th>
				<th>Preço</th>
			</tr>
		...
		<span ng-show="(cardapio | filter:criterio).length == 0">Infelizmente
		...
		<form name="pedidoForm" class="form-inline" novalidate>
			<input type="number" class="form-control" style="width: 130px;"
				name="codigo" ng-model="pedido.codigo" placeholder="Código da Mesa"
				ng-required="true" ng-minlength="2" ng-maxlength="2" /> <select
				class="form-control" style="width: 400px;" name="item"
				ng-model="pedido.item"
				ng-options="item as item.descricao for item in (cardapio | filter:criterio)"
				ng-required="true"><option value="">Selecione um
					item do cardápio...</option></select>
			<button class="btn btn-default" ng-click="pedir()"
				ng-disabled="pedidoForm.codigo.$invalid || pedidoForm.item.$invalid">Fazer
				Pedido!</button>
		</form>
		<span ng-show="pedidoForm.codigo.$dirty && pedidoForm.codigo.$invalid">O
			campo Código da Mesa é obrigatório e deve ter 2 caracteres no máximo</span>
		<br />
